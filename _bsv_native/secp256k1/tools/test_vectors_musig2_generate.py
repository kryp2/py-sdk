...
# key agg vectors
with open(sys.argv[1] + "/key_agg_vectors.json") as f:
    data = json.load(f)

    max_key_indices = max(len(test_case["key_indices"]) for test_case in data["valid_test_cases"])
    max_tweak_indices = max(len(test_case["tweak_indices"]) for test_case in data["error_test_cases"])
...
""" % (
    ...
    s += init_cases(
        ...
        lambda case: "{ %s, { %s }}," % (init_indices(case["key_indices"]), hexstr_to_intarray(case["expected"])),
    )
...
# nonce gen vectors
with open(sys.argv[1] + "/nonce_gen_vectors.json") as f:
    data = json.load(f)

    # The MuSig2 implementation only allows messages of length 32
    data["test_cases"] = list(filter(lambda c: c["msg"] is None or len(c["msg"]) == 64, data["test_cases"]))
...
# nonce agg vectors
with open(sys.argv[1] + "/nonce_agg_vectors.json") as f:
    data = json.load(f)
...
# sign/verify vectors
with open(sys.argv[1] + "/sign_verify_vectors.json") as f:
    data = json.load(f)
    ...
    max_nonce_indices = max(
        len(test_case["nonce_indices"]) if "nonce_indices" in test_case else 0 for test_case in all_cases
    )
    # Add structures for valid and error cases
    s += """
...
# tweak vectors
with open(sys.argv[1] + "/tweak_vectors.json") as f:
    data = json.load(f)
...
# sigagg vectors
with open(sys.argv[1] + "/sig_agg_vectors.json") as f:
    data = json.load(f)
